...
class OptimizerVAE(object):
    def __init__(self, preds, labels, model, num_nodes, pos_weight, norm, lr):
        ...
        self.cost = norm * tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(logits=preds, labels=labels, pos_weight=pos_weight))
        self.optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=lr)  # Adam Optimizer，随机梯度下降，beta1：一阶矩估计的指数衰减率；beta2：二阶矩估计的指数衰减率；epsilon：该参数是非常小的数，其为了防止在实现中除以零。默认参数设定为：alpha=0.001、beta1=0.9、beta2=0.999 和 epsilon=10E−8
